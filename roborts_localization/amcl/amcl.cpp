...
/*     max_uwb_particles_ = amcl_param_.max_uwb_particles;
    ...
    std::cout<<"【amcl Init】完成"<<std::endl;
    LOG_INFO << "Amcl Init!";
  }
  ...
  void Amcl::HandleMapMessage(const nav_msgs::OccupancyGrid &map_msg, const Vec3d &init_pose, const Vec3d &init_cov)
  {
    ...
    odom_model_ptr_ = std::make_unique<SensorOdom>(amcl_param_.odom_alpha1,
                                                   ...
                                                   amcl_param_.odom_alpha5); //odom运动噪声
    ...
    LOG_INFO << "Done initializing likelihood field model."; //完成初始化似然域模型初始化

    if (use_global_localization_)//为ture进行均匀分布初始化
    {
      GlobalLocalization();
    }
    else//否则用高斯分布初始化
  ...
  void Amcl::UpdatePoseFromParam(const Vec3d &init_pose, const Vec3d &init_cov)
  {
    ...
    DLOG_INFO << "Updated init pose " << init_pose_; //更新初始位姿
  }
                                      ...
                                      Mat3d pf_init_pose_cov)
  {
    ...
    initial_pose_hyp_->pf_pose_mean = pf_init_pose_mean; //先验位姿
    initial_pose_hyp_->pf_pose_cov = pf_init_pose_cov;   //先验位姿协方差
    ApplyInitPose();
  }
  ...
  bool Amcl::GlobalLocalization()
  {
    ...
    LOG_INFO << "Initializing with uniform distribution"; //用均匀分布初始化
    ...
    LOG_INFO << "Global initialization done!"; //全局初始化完成
    pf_init_ = false;
    return true;
  }
  ...
  int Amcl::Update(const Vec3d &pose,                        //更新odom
                   const sensor_msgs::LaserScan &laser_scan, //更新激光雷达
                   ...
                   const double *updatearr)
  {
    ...
    update_tf_ = false;
    UpdateOdomPoseData(pose, updatearr);

    resampled_ = false;
    if (laser_update_)
    {
      UpdateLaser(laser_scan, angle_min,
                  angle_increment,
                  pose,
                  particle_cloud_pose_msg,
                  updatearr);
    }
                                ...
                                const double *updatearr)
  {
    ...
    if (!pf_init_)
    {
      // Pose at last filter update（最后更新滤波的位姿）
      pf_odom_pose_ = pose;
      // Filter is now initialized"（粒子滤波已初始化）
      pf_init_ = true;
    ...
    else if (pf_init_ && laser_update_)
    {
      ...
      odom_model_ptr_->UpdateAction(pf_ptr_->GetCurrentSampleSetPtr(),
                                    odom_data,
                                    updatearr);
    }
  }
//更新测量步
                         ...
                         const double *updatearr)
  {
    ...
    laser_update_ = false;
    pf_odom_pose_ = pose;
    particle_cloud_pose_msg = ResampleParticles(updatearr);
  }

  geometry_msgs::PoseArray Amcl::ResampleParticles(const double *updatearr)
  {
    geometry_msgs::PoseArray particle_cloud_pose_msg;
    //重采样，注入新粒子相关
    if (!(++resample_count_ % resample_interval_))
    {
      ...
      pf_ptr_->UpdateResample(updatearr);
      resampled_ = true;
    }
                          ...
                          ros::Time laser_msg_timestamp)
  {
    ...
    if (resampled_ || force_publication_)
    {
      ...
      if (max_weight > 0.0)
      {
        ...
        publish_pose_ = true; //发布amcl位姿（hyp_pose.pose_mean）
        update_tf_ = true;
      }
      else
      {
