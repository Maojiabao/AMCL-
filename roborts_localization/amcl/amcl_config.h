...
struct AmclConfig {
  void GetParam(ros::NodeHandle *nh) {
      ...
      nh->param<bool>("first_map_only", use_map_topic, false);
      ...
      nh->param<int>("min_particles", min_particles, 500);       //最小粒子数500
      nh->param<int>("max_particles", max_particles, 5000);      //最大粒子数5000
      ...
      nh->param<double>("laser_likelihood_max_dist", laser_likelihood_max_dist,  5.0);
      nh->param<bool>("do_beamskip", do_beamskip, true);
      nh->param<double>("beam_skip_distance", beam_skip_distance, 0.2);
      nh->param<double>("beam_skip_threshold", beam_skip_threshold, 0.3);
      nh->param<double>("beam_skip_error_threshold", beam_skip_error_threshold, 0.99);
      ...
      nh->param<double>("odom_alpha5", odom_alpha5, 0.05);
      nh->param<double>("update_min_d", update_min_d, 0.05);
      nh->param<double>("update_min_a", update_min_a, 0.1);
      nh->param<int>("resample_interval", resample_interval, 1);
      nh->param<double>("transform_tolerance", transform_tolerance, 0.1);
      nh->param<double>("recovery_alpha_slow", recovery_alpha_slow, 0.01);
      nh->param<double>("recovery_alpha_fast", recovery_alpha_fast, 0.1);
      nh->param<bool>("use_global_localization", use_global_localization, false);
      nh->param<bool>("random_heading", random_heading, true);
