...
namespace roborts_localization
{
  ...
  void ParticleFilter::InitByGuassian(const Vec3d &mean, const Mat3d &cov)
  {
    ...
    for (i = 0; i < sample_set_ptr->sample_count; i++)
    {
      ...
      sample_set_ptr->samples_vec[i].pose = pf_gaussian_pdf.GenerateSample(); //生成随机位姿的样本
  ...
  void ParticleFilter::UpdateResample(const double *updatearr)
  {
    ...
    while (set_b->sample_count < this->max_samples_)
    {
      ...
      if (drand48() < w_diff)
      {
        ...
        //根据当前位置生成粒子（均匀分布）
        //当位于地图下半部分时（X:0.2~8.28,Y:0.2~2.44）
        if (*(updatearr + 1) > 0.2 && *(updatearr + 1) <= 2.44 && *updatearr > 0.2 && *updatearr < 8.28)
        {
          sample_b->pose[1] = (*(updatearr + 1) + 2.44) / 2.0 + math::RandomEquidistributionNum<double>((*(updatearr + 1) + 2.04) / 2.0);
          if (*updatearr > 0.2 && *updatearr <= 2.22)
          {
            sample_b->pose[0] = (*updatearr + 2.22) / 2.0 + math::RandomEquidistributionNum<double>((*updatearr + 1.82) / 2.0);
            //当位于地图两侧时粒子发散角度为全方向
            sample_b->pose[2] = *(updatearr + 2) + math::RandomEquidistributionNum<double>(1.0 * 3.14);
          }
          else if (*updatearr > 2.22 && *updatearr < 6.26)
          {
            sample_b->pose[0] = *updatearr + math::RandomEquidistributionNum<double>(2.02);
            //当位于场地中间时。发散粒子的角度为左右各90度
            sample_b->pose[2] = *(updatearr + 2) + math::RandomEquidistributionNum<double>(0.5 * 3.14);
          }
          else if (*updatearr >= 6.26 && *updatearr < 8.28)
          {
            sample_b->pose[0] = (*updatearr + 6.26) / 2.0 + math::RandomEquidistributionNum<double>((10.3 - *updatearr) / 2.0);
            sample_b->pose[2] = *(updatearr + 2) + math::RandomEquidistributionNum<double>(1.0 * 3.14);
          }
        }
        //当位于上半部分时（X:0.2~8.28,Y:2.44～4.68)
        else if (*(updatearr + 1) > 2.44 && *(updatearr + 1) < 4.68 && *updatearr > 0.2 && *updatearr < 8.28)
        {
          sample_b->pose[1] = (*(updatearr + 1) + 2.44) / 2.0 + math::RandomEquidistributionNum<double>((6.92 - *(updatearr + 1)) / 2.0);
          if (*updatearr > 0.2 && *updatearr <= 2.22)
          {
            sample_b->pose[0] = (*updatearr + 2.22) / 2.0 + math::RandomEquidistributionNum<double>((*updatearr + 1.82) / 2.0);
            //当位于地图两侧时粒子发散角度为全方向
