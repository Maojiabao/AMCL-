...
class ParticleFilter {
 ...
 public:
  ...
  /**
   * @brief  初始化      Initialize the filter using a guassian pdf
  ...
  /**
   * @brief 初始化     Initialize the filter using some model
  ...
  /**
   * @brief 重采样    Resample the distribution
   * @param re_x
   * @param re_y
   * @param re_yaw
   */
  void UpdateResample(const double *updatearr);
   ...
   * @return 返回重采样所需数量 Returns the required number of samples for resampling.
   */
  ...
  /**
   * @brief Compute the statistics for a particular cluster.计算特定群集的统计信息
 ...
 private:
  ...
  //! 样本集 The sample sets.  We keep two sets and use [current_set] to identify the active set.
  int current_set_;
