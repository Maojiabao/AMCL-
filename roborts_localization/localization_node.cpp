...
//如果地图信息和激光雷达都获得到，就ture
  bool LocalizationNode::Init()   
  {
   ...
   //发布的话题 （地图  粒子云 Mp定位点   fort_mode）
    ...
    amcl_ptr_->Init(init_pose_, init_cov_);//将amcl内的init_pose初始化
...
//获取静态地图  获取成功则调用 AMCL::HandleMapMessage，这个函数是主要的模型处理函数
//它根据地图信息创造了地图空闲区域、粒子滤波器、里程计运动模型与激光感知的似然域模型
  bool LocalizationNode::GetStaticMap()
  {
    ...
    if (static_map_srv_.call(req, res))
    {
      ...
      //AMCL::HandleMapMessage将位姿保存在容器pose[] 和 cov[][]中
...
//得到雷达位姿    雷达坐标系在base上的位置   laser_pose  
  bool LocalizationNode::GetLaserPose()
  {
    ...
    cout<<"【雷达的位姿】获取成功"<< laser_pose[0] << ", " << laser_pose[1] << ", " << laser_pose[2]<<endl;
...
// 它将休眠并重试每个polling_duration，直到超时的持续时间已经过去。
      ...
      ros::Time now = ros::Time::now();//odom在base下坐标
    ...
    tf::Pose pose_new;
    tf::Pose pose_old;
    tf::poseMsgToTF(init_pose_msg->pose.pose, pose_old);//map下的pose
    pose_new = pose_old * tx_odom;//pose在base下坐标
...
//调用TransformLaserscanToBaseFrame    GetPoseFromTf从TF树上获取某时刻的位姿 AMCL::update更新信息
  void LocalizationNode::LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg_ptr)
  {
    ...
    // cout<<"【激光雷达】开始角度"<<angle_min<<"      增加的角度"<<angle_increment<<endl;//没意义，这是个固定值：-1.8/0.3333
    //如果雷达正常工作
    if (laser_act)
    {
      last_laser_msg_timestamp_ = laser_scan_msg_ptr->header.stamp;//雷达话题设置为最新时间
      first_theta_stamp = ros::Time::now();
      get_first_theta = true;

      Vec3d pose_in_odom;//在最新时间内，将机器人坐标下的点转换为odom坐标下的点------把此当作里程计使（机器人坐标系是不断变化的）
      ...
      amcl_ptr_->Update(pose_in_odom,
                        ...
                        particlecloud_msg_,
                        hyp_pose_,
                        updatearr_);//更新base--odom,更新测量步，更新粒子云，更新最终位姿
...
//发布到可视化
  ...
  {
    //？？？？？？
    ros::Time transform_expiration = (last_laser_msg_timestamp_ + transform_tolerance_);//推迟发布坐标边患？
  ...
  // 从TF树上获取某时刻的位姿//source坐标下的点  转化到target坐标系下  在timestamp时间内  最后获得点pose[3]为转换后的点
